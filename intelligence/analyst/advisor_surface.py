...
# ── the LIFE-PHASE memo (T-346) ───────────────────────────────────────────────
# The census (2026-08-27) made a flat ranked list the wrong shape: three years of law
# school sit between now and the long horizon, so the SAME move is right, wrong, or
# impossible depending on phase. Structure follows the facts, not a template.
#
# SCOPE (user directive 2026-08-27): this memo INFORMS, it does not STEER. It is a
# SIDEBAR — it does not re-prioritise the program, and no program surface leads with
# contribution/wrapper framing. The program's centre stays Act 2 + the AI layer.

SCOPE_NOTE = (
    "**This memo is a sidebar, not a steering input.** It quantifies wrapper and "
    "contribution options the user owns. It does **not** re-prioritise the research "
    "program — the centre of gravity remains Act 2 + the AI layer — and no other "
    "surface leads with this framing. Rare refresh by design."
)


def render_memo(census: dict, r: float = 0.07, as_of: str = "",
                tiers=DEFAULT_TIERS, extra: float = 1000.0) -> str:
    """The phase-structured advisor memo, generated from the census facts."""
    cap = (census.get("capacity") or {})
    extra_cap = float(cap.get("extra_annual_capacity") or 0)
    accts = {a["account_type"]: a for a in census.get("accounts", [])}
    opens = census.get("open_inputs", [])

    out = [f"# Advisor memo — {as_of}", "",
           "*Informational only. It does not recommend a date, schedule anything, or "
           "urge an action.*", "", SCOPE_NOTE, ""]

    # ---- the top open input comes FIRST: everything else is downstream of it ----
    if opens:
        out += ["## ⚠ The top open question — asked, not assumed", ""]
        for q in opens:
            out += [f"**{q['question']}**", "", f"{q['why']}", "",
                    f"*Status: {q['status']}.* Until it is answered this memo cannot "
                    f"responsibly rank anything that competes with liquidity — so it "
                    f"does not try.", ""]

    out += ["## The liquidity flag (stated before any optimisation)", "",
            "**Money earmarked for tuition and living costs during school must NOT sit "
            "in the 40-year maximum-terminal-wealth machine.** That machine is built to "
            "ride drawdowns for decades; a bill that arrives on a fixed date is the one "
            "thing it is structurally unsuited to fund. This is a constraint, not a "
            "preference, and it outranks every optimisation below.", ""]

    # ---- phases ----
    out += ["## Phase 1 — NOW (pre-school)", ""]
    roth = accts.get("roth", {})
    out += [f"- **Roth: already maxing (~{_money(roth.get('annual_contribution', 0))}/yr).** "
            f"Captured — there is no move here, which is the correct outcome to report.",
            "- **401(k): none today**; eligibility possible in ~2 years at the current "
            "employer, and **no match exists to capture now**. That makes it a "
            "*calendar trigger to revisit at eligibility*, not an available move.",
            "- **HSA: eligible in principle — the top *current* wrapper item, conditional "
            "on one fact.** If the health plan is HDHP-qualified, an HSA is the only "
            "triple-tax-advantaged wrapper available (deductible in, compounds untaxed, "
            "qualified withdrawals untaxed) and is strongest when contributions are "
            "**invested rather than spent**. **Verify HDHP status first** — if the plan "
            "does not qualify, this item disappears entirely.", ""]

    out += ["## Phase 2 — DURING SCHOOL (~3 years)", "",
            "- **Contribution capacity likely falls to ~zero.** The additional capacity "
            f"below ({_money(extra_cap)}/yr) is realistically a **pre- and post-school** "
            "figure, not a during-school one. Treating it as continuous would overstate "
            "the machine's runway by three years of contributions.",
            "- **Low-bracket years are themselves an asset.** Two mechanisms only "
            "available while income is low: **tax-gain harvesting** (realising long-term "
            "gains at a 0% bracket to reset basis upward) and **low-bracket Roth "
            "conversions**. Both are certain-sign and both disappear when income "
            "recovers. That is a fact about the tax code and the calendar, stated so it "
            "is known — not a prompt to act on any particular timeline.",
            "- **The liquidity constraint above binds hardest here.**", ""]

    out += ["## Phase 3 — POST-SCHOOL", "",
            "- **Income likely exceeds the Roth phase-out** → direct Roth contributions "
            "stop being available and the **backdoor Roth** becomes the relevant "
            "mechanism (~year 4–5 on the current timeline).",
            "- **A 401(k) with a match almost certainly appears.** An unclaimed match is "
            "the single highest-certainty item in this entire memo whenever it exists — "
            "it is an immediate return, not a forecast. It does not exist today.",
            "- Capacity returns, and the contribution arithmetic below applies in full.", ""]

    # ---- the quantification, per phase ----
    out += ["## What the extra capacity is worth", "",
            f"The census reports **{_money(extra_cap)}/yr** of additional capacity "
            f"('could be more'). Expressed as the exchange rate that makes it comparable "
            f"to research output — **the annual alpha that would match it**:", ""]
    rows = sensitivity_table(10_000, 7_000, r, extra=extra_cap or 5000.0)
    out += ["| horizon | added terminal wealth | ≡ alpha of |", "|---|---|---|"]
    for x in rows:
        eq = "—" if x["equivalent_alpha_bps"] is None else f"**{x['equivalent_alpha_bps']:.0f} bps/yr**"
        out.append(f"| year {x['year']} | {_money(x['delta_dollars'])} | {eq} |")
    out += ["", "*Assumed $10,000 starting balance, $7,000/yr Roth, 7%/yr — the balance "
                "and return are ASSUMPTIONS, not the user's figures. Phase 2 above is the "
                "reason these are not a straight-line projection.*", "",
            "**For scale:** every free-data return-frontier probe this program has run "
            "closed H0, and the one statistically-significant alpha found (CEF discount "
            "capture, t_HAC 2.31) has no retail data path. A contribution increase is "
            "certain-sign; an edge of the same size is not evidenced anywhere here. "
            "That comparison is the memo's only real argument.", ""]
    return "\n".join(out) + "\n"
