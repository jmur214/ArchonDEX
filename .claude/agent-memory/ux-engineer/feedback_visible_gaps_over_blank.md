...
**Honest-framing pattern (2026-07 Paper-tab rework, validated):** when a panel's framing is load-bearing (e.g. "legacy backtest — H0, NOT deployed", "paper validates EXECUTION ONLY, never edge"), put it in a module-level string constant and render it in BOTH the found and the pending branch of the render helper — the honesty must survive the empty state, and tests can assert on the constant. Static pointer/honesty cards go in the layout (visible before any callback fires); the callback output carries only a compact one-line framing note so the copy isn't duplicated on screen. Per the user's Sortino/tail directive, metric tables in the cockpit are MaxDD-first, Sharpe/ci_low demoted after MaxDD+CAGR (kept, never dropped — ci_low is still required).
...
- Equity-vs-robo (2026-07 correction, T-188 label drift fixed): the preferred path is `data/paper_state/equity.csv` (NOT `data/paper/latest/` — that was a doc-drift guess), and NOTHING produces that file yet — the fallback to newest `portfolio_snapshots.csv` labelled "(backtest base — paper returns pending)" IS the live path. `build_scorecard(base)` from `core.combined_candidate_scorecard` is SLOW (block-bootstrap) → call with small `n_boot` (~300) and lru_cache by source-file mtime so the 2s pulse never recomputes per tick. Post-T-238 this table is a LEGACY reference (old equity book, H0, not deployed), never a "deploy gate". Heartbeat key is `account_explained` (T-198 renamed from `account_flat`; read with fallback).
