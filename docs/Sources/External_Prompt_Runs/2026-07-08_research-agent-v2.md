...
| # | Finding | Disposition |
|---|---|---|
| 1 | Leverage-vehicle under-tested (SSO decay vs NTSX/RSSB negative-cost stacking) | **DISPATCHED → D/T-294 — RAN 2026-07-08, SPLIT verdict:** decay mechanism CONFIRMED (+0.86%/yr leaks through SSO, chop-decay-driven, not ER/financing) but "NTSX/RSSB = a stronger gated-2× vehicle" is **REFUTED** (0.9×/1.0× equity caps → gated they LOSE to buy-hold SPY; their Sortino edge is allocation, not vehicle). No Roth vehicle recovers the gap. Displacing finding: the config is EXECUTION-BOUND (slippage at the measured ≥5bps SSO floor exceeds the whole +0.45%/yr edge) → real money HELD pending E's measured number + T-297 turnover reduction. |
