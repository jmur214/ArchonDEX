...
**⚠️ T-092's 0.246 STANDS (T-126); NEW CRITICAL PATH = pin the DATA substrate (T-127):** The 0.446 was a **DATA-DRIFT artifact**, not a real re-baseline. T-125 fixed the base-layer pin (3/3 determinism within a build), but T-126's *fresh build 3 days later* on identical pinned base + identical HEAD gave arm0 26-yr **0.246** again (reproduces T-092 to 3 decimals) — because `Dockerfile.backtest:105-106` bakes `data/processed/`+`data/raw/` from the **host filesystem UNPINNED**, so the substrate silently drifts across builds. **The bull-conditional collapse is REAL — no softening.** The held cloud wave stays held until the data substrate is pinned (T-127). Methodology note: a single-cell "OFF canon-identical" proof does NOT prove 26-yr inert (the T-101/111/116/118/120 inert merges moved 26-yr by a benign ~0.009 — confirmed Cell A vs B).
