...
- **T-098 H-Band no-trade bands** — REFUTED at ±20%/±25% on 12-yr (Δ Sharpe +0.008/+0.018, ci_low NEGATIVE both; turnover Δ <2% vs predicted −60-70%; skew mixed, arm2 worse). Band suppresses small rebalances but not the dominant large vol-target ones. Clean Engine C impl (default-OFF, canon-OFF=baseline, ON-differs, det 3/3) kept additive for a future tighter-band sweep. Branch `feature/h-band-no-trade-t098` @ bf9da13 NOT merged (awaiting user — refuted+inert, no urgency). Audit: `no_trade_band_h_band_t098_audit_2026_05_31.md`. **NOTE: B's harness reports 12-yr arm0_off Sharpe 1.314 — this is MEAN-OF-12-ANNUAL Sharpes, NOT comparable to the pooled-full-window 0.81 (same window). Director-verified from the cell JSON; the 0.81 baseline STANDS. Same statistic-mismatch that made the old Foundation-Gate 1.296 collapse to pooled 0.507.**
