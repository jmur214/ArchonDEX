...
>
> The bought managed-futures separate-account sleeve is the FIRST working crisis-defense in the whole arc: T-170 (recent window: MDD −7.5%→−5.6%, +25.1%; 2022 DBMF +32.7%/KMLM +48.8%) + T-171 (deep windows, net-of-haircut via the free AQR TSMOM proxy — **director-corrected, see below**: dotcom −19.0%→−11.8%/−13.5% (cut 38%/29%, clears ≥25% both haircuts); GFC −30.2%→−21.9%/−23.6% (cut 27%/22% — clears the PRIMARY haircut, FAILS the conservative; 30% needed for a robust GFC cut at 42%/33%)). It is a MEASURED DRAWDOWN-defense across all four crises (dotcom/GFC/COVID/2022), NOT a proven Sharpe-lifter (Sharpe ci_low indeterminate on thin crisis samples). **CORRECTION (2026-06-15): T-171's published combined-MDD cells were ~2× overstated by a combination-step bug; an adversarial verification + independent director recompute (from the S3 base curve + public AQR) produced the corrected numbers above — the GFC@20% ≥25% bar is haircut-FRAGILE, and all figures are an optimistic ceiling vs the real DBMF/KMLM products (replication lag distorts crisis shape). Fix dispatched as T-173.** Both IN-HOUSE levers stay dead (de-gross T-118r, capital-partitioned sleeve T-128r). This resolves the crisis-defense question into two layers — **do NOT re-litigate it as "crashes are unpredictable":**
>
...
>
> **T-172 STEP 1 RESULT (2026-06-16):** the regime detector is **REGIME-CLASSIFICATION-grade, NOT sharp-timing-grade.** Leave-one-crisis-out (causal forward-filter) GENERALIZES OOS to fast/credit crises (GFC 333td lead but FA 1.13/yr > budget; COVID 17td; 2022 108td; calm silent) — the dotcom-blindness was the data-floor + the VIX-term-structure features (inception 2007/2011), not a fundamental inability — BUT it stays WEAK on the slow dotcom bear (catches the fast Apr-2000 leg, misses the 2001 de-rating grind; partly structural). **This is the key refinement: classification-grade EXPLAINS why de-gross (T-118r) failed (it needed precise flatten-timing the signal lacks) and validates the SLEEVE-SIZER form of the amplifier (you want MF heavier through the whole risk-off regime, which a sustained signal supports).** So the amplifier is a **fast-crisis regime sizer**: crash-timing is viable for fast crises (always-on 20% is NOT the ceiling there); always-on stays the ceiling for the slow-valuation-bear. Step 2 (dynamic sizer A/B vs always-on, OOS net-of-cost) is gated + scoped; the GFC FA-budget breach means the Step-2 operating point needs T-152-style calibration. **OOS-causality adversarial verification in flight before the Step-2 greenlight** (this project has a regime-validation lookahead history — T-087/T-089).
