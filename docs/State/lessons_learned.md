...
This document serves as the historical memory of ArchonDEX for all developers (AI and human). 
...
- **2026-08-15 | The channel a new consumer reads can be structurally EMPTY while every producer reports healthy — check the INPUT's actual contents before igniting, not just that the pipe exists (T-329b, account-3 pre-ignition) |** Account-3 (the stage-2 AI trader) consumes the constrained analyst's `hypothetical_actions` and turns them into real paper orders. Everything was built, tested (40 new tests), provisioned, and the inherited Alpaca account was verified FLAT from the broker. The last pre-ignition check — run the REAL constructor against the REAL notes in S3 — returned an empty plan. Cause: **0 of 15 notes carry a single hypothetical action.** The notes themselves are rich (market assessment, risk flags, 3 machine-checkable predictions each); the model simply never uses that field, and the `daily_v2` prompt invites exactly that — it calls the actions "SMALL exploratory satellite tilts", states "**They are never executed**", and ends "**Omit the whole list if you have no small-tilt view.**" The model complied, every day. **Blast radius beyond account-3: C's `LlmShadowBook` — the virtual rehearsal of this exact stream — has 14 accrued points, every one `n_actions: 0, turnover: 0, positions: {}, book_nav: 1.0`, against a twin that compounded to 1.0329, and it reported `degraded: false, action: "applied"` every single day.** A 14-day A/B has been comparing a permanently-100%-cash book to a compounding twin and calling it clean. **Lessons, three:** (1) `[NN-FIRST-ARTIFACT]` must be applied to the INPUT as well as the output — "the note file exists and validates" is not "the field this consumer reads is populated"; the clock-census asks *did the artifact advance*, and every note DID advance, which is why nothing caught it. (2) A prompt that says an output is "never executed" is a **behavioural instruction**, and wiring that output to a real order path silently invalidates the premise the analyst was operating under — you cannot change the consumer without re-reading what the producer was told. (3) An always-empty channel is the most dangerous shape of all because it degrades NOTHING: the record reads canonical, `reject_reason: none`, `degraded: false`, zero orders, forever. Distinguish "nothing to do" from "nothing was ever asked for" **in the artifact**, or a null record will pass as a healthy one. Sixth instance of the stalled/empty-clock class (eval loop on an empty set, frozen price CSV, empty news tape ×2, ungated feeds, denied S3 pushes) and the first found by checking the input rather than the output.
- **2026-05-22 | Testing-setup confounds can mask real edges — T-041b spinoff Gate 1 measured a 0.25x → 1.25x contribution delta instead of clean 0 → 1.0 |** Agent A's T-041b shipped the spinoff edge through the gauntlet and got FAIL Gate 1 (contribution Sharpe +0.000 vs +0.10 threshold). Honest diagnostic: spinoff_reversion_v1 was already auto-registered to the paused tier (0.25× weight) from T-041, so Gate 1's contribution-delta measurement was 0.25× → 1.25× (mostly baseline noise) instead of the clean 0 → 1.0× delta Gate 1 was designed for. Plus only ~24 of 150 spin-off children had cached OHLCV in the validate-effective window. **Lesson**: when a candidate edge is being gauntleted, the gauntlet's contribution measurement must be FROM ZERO not FROM-paused-baseline. Future Engine F lifecycle work should ensure candidates being gauntleted are NOT in the paused tier during the test. The shape of this bug is the same shape as 2026-05-10 T-019 "paused-tier inert" finding — paused-tier interaction with measurement obscures real signal. **The verdict says "this configuration doesn't show measurable contribution," not "spin-offs don't generate alpha."** T-041c follow-ups queued: archive paused entry + re-run, hyperparameter sweep, pair-trade variant, microcap substrate when Norgate gate clears.
- **2026-05-22 | Engine B vol-targeting propose-first discipline: T-055 ships INERT default-OFF, T-055b flag-flip is separate sub-dispatch gated on T-055c lift verification |** B's T-055 demonstrated the right propose-first pattern for Engine B work. Three layers: T-055 = code ship + INERTNESS gate (defense-first enabled=False; canon md5 bitwise identical to T-019 reference). T-055c = lift verification A/B campaign (~6 hr, full 5-yr × 3-rep × 2-arm grid). T-055b = production flag-flip (gated on T-055c showing the Moreira-Muir +0.10-0.20 Sharpe lift materializes). Each is a separate user-approval gate. **Lesson**: for Engine B / live_trader / any highest-blast-radius code, the propose-first discipline is best implemented as THREE explicit dispatches: code-ship-inert, lift-verify, prod-flip. Even when the original user-approval covers "implement this," the prod-flip should be its own user-decision gate. B carried this discipline correctly and named it explicitly in the outbox.
- **2026-05-12 | Engine D's vocabulary expansion was a dead-letter office — production hunt() never passed ticker= to compute_all_features |** Agent B surfaced 2026-05-12 evening during T-038-CONT vectorization investigation. Empirical: production hunt() on 53 tickers × 1yr completes in 20.5 sec emitting 3 candidates; the smoke's 65min silent CPU was in a DIFFERENT, still-unprofiled code path. **The foundry_feature gene type has been structurally unable to produce candidates with computed feature values since the project arc began.** This explains T-021's "all 3 candidates were rsi_bounce_v1 mutations" (the GA emitted foundry_feature genes correctly per T-022; they just referenced columns that never got populated in production hunt() → trivial Gate 1 fails). T-025's 30/30 Gate 1 kill rate was not signal weakness but structural plumbing failure. T-052's 4 new regime features are currently dead-on-arrival in production until the wiring lands. **The "Foundry features invisible to GA gene encoding" diagnosis from 2026-05-11 was correct but the locus was off — gene encoding was fine; the FEATURE COMPUTE in production was the gap.** Lesson: when a system component appears to be performing badly, check whether its plumbing actually invokes the component before concluding the component itself is the problem. Highest-leverage forward dispatch: T-054 wires the ticker= argument through; unblocks T-022 + T-023 + T-024 + T-038-CONT + T-052 cumulatively in <1-2 hr fix time. Parallels the cockpit bug pattern in shape (silent structural gap masked as functional weakness).
- **2026-05-12 | Engine F lifecycle factor-α gate fires on 6 of 7 cockpit-fixed actives — but ci_low captures what point estimate misses |** T-043 shipped the symmetric retirement gate (matching Discovery Gate 6). Re-eval result: 6/7 fire (every UNIFORMLY-NEGATIVE active retires; STR keeps as UNIFORMLY NOISY with ci_low above threshold). **Critical pattern**: gap_fill_v1 and volume_anomaly_v1 fire on **ci_low alone** — point estimate -0.93 is ABOVE the -2.0 threshold, but ci_low at -3.9/-4.0 reveals the real downside exposure. This is precisely the CLAUDE.md 6th non-negotiable working as designed; if the gate had been point-estimate-only, both edges would have falsely passed. Lesson: the ci_low rule isn't just doctrinal — it materially changes lifecycle decisions when noise straddles a threshold. Default-disabled until user reviews + flips the flag (auto-retiring 6 of the active set is a significant production-state change requiring explicit approval).
- **2026-05-12 | Dollar PnL ≠ factor-adjusted alpha — "winning" edges can be UNIFORMLY NEGATIVE on α |** Morning of 2026-05-12, the 2024 attribution dive (commit d1ed01f) labeled `volume_anomaly_v1` and `gap_fill_v1` as "5-year winners" based on +$4,527 and +$3,410 of realized dollar PnL. Evening of 2026-05-12, T-036 Part B (per-regime factor decomp on cockpit-fixed logs) revealed both are UNIFORMLY NEGATIVE on factor-adjusted α (t = -2.06 to -2.77 in expansion regimes). The positive dollar PnL was Mkt+Mom factor beta exposure; the residual α after factor subtraction is significantly negative. Lesson: when evaluating an edge's quality, dollar PnL is necessary but not sufficient. The system can be making money WHILE having no idiosyncratic alpha — if the dollar PnL is just inherited factor exposure. Two lenses (raw Sharpe / factor-adjusted α) need to be held simultaneously. Engine F lifecycle gates, T-043 retirement decisions, and Discovery's gauntlet all need to be explicit about which lens applies. This is the inverse of the 2024 attribution dive's framing — its dollar-PnL framing was right but incomplete.
- **2026-05-12 | Bi-directional metric-pipeline contamination — winning years can be over-reported too |** T-035 corrected T-002's 0.270 baseline to 0.598 after applying the cockpit metrics-pipeline fix (T-034). My pre-T-035 prediction "the bug barely fires in small-MDD cells, expected shift 0.02-0.05" was REFUTED. Mechanism: `pandas.read_csv` was mis-aligning columns because the writer emitted 11 fields against a 9-column header; `peak_equity` (monotone non-decreasing) landed in the `equity` slot. **Winning years:** peak_equity has lower variance than real equity (real equity dips during intra-year drawdowns), so reading peak-as-equity SYSTEMATICALLY INFLATED Sharpe (2021: 0.413 reported → 1.791 corrected). **Losing years:** peak stays at starting capital while real equity falls, looking like flat equity → Sharpe ≈ 0 (2024: 0.236 reported → -0.613 corrected). Lesson: when investigating a metric-pipeline bug, don't reason from "where the obvious effect would be largest." A monotone-series-in-noisy-slot bug is bi-directional even when intuition says one direction. Verify per-year, not just "obvious failure" cells. Closes the same loop as the 2026-05-07 lazy-determinism-reset lesson: "the bug is where you didn't look."
- **2026-05-12 | Bull-conditional with material 2024-style downside is now load-bearing | ** Corrected per-year reads: 2021 1.791, 2022 0.294, 2023 1.221, 2024 -0.613, 2025 0.297. The -0.613 2024 cell is real fragility that the bug had hidden. Implication: the engines-first directive's projected +0.55 to +1.25 lift from engine completion is anchored to a baseline (now 0.598, not 0.270) that has a 1-in-5-year downside event embedded in its own per-year record. Regime-conditional defensive layer (Engine B regime-driven de-grossing, Engine E HMM input panel rebuild) is no longer purely speculative future work; it's the response to a visible failure mode in the corrected canonical baseline.
- **2026-05-11 | Engine D gene encoding is the gating constraint for Discovery — vocabulary expansion delivers nothing without it |** T-021 (first-ever Discovery cycle on substrate-honest) produced 3 candidates, ALL `rsi_bounce_v1` mutations. The post-T-006 Foundry features (mom_12_1, vol_regime, dist_52w_high, hyg_lqd_spread, calendar/regime/macro features) and post-T-014 calendar features (FOMC drift, sell-in-May, etc.) are invisible to GA's gene encoding. Lesson: **adding features to the Foundry doesn't add candidate-search reach unless Engine D's gene encoding is extended to emit candidates that consume those features.** Implication for future "vocabulary expansion" framing: always pair with "and Engine D consumes this via gene encoding [X]" or it's a write-only feature library. Highest-leverage Engine D work going forward.
- **2026-05-11 | Gate 1 caching is the leverage point for tractable Discovery cycles |** T-021's per-candidate wall-time was 3,240-6,689 sec because Gate 1 runs a full ModeController backtest per candidate. Cap=30 candidates = 37+ hr — infeasible. Cached signal-collector replay (compute active-ensemble signal stream ONCE per (universe, window), then replay candidate contributions layered on top) would deliver 10-50× speedup per B's estimate. T-013's Foundry-loop vectorization doesn't help here (different code path). Lesson: Discovery cycle scalability is a separate axis from feature-engineering vectorization; they share the "vectorize" mindset but the load-bearing optimizations are distinct.
- **2026-05-11 | 0/11 edges clear FF5+Mom t > 2 on substrate-honest substrate — universal pattern |** T-004 (0/6 actives) + T-020 (0/5 new paused) = 0/11 total. Raw Sharpe in this system is Mkt + Mom factor exposure, not idiosyncratic alpha. The pattern is universal across the entire edge inventory tested. Lesson: this either means the t > 2 bar is incompatible with retail-scale substrate-honest universes (worth director-side threshold-calibration analysis, not goalpost-moving since no Sharpe-claim is being made) OR the project genuinely has no factor-adjusted alpha at this scale. Either way it informs the realistic Sharpe-lift ceiling: if no individual edge has factor-adjusted alpha, the lift has to come from ENSEMBLE interactions, regime conditioning, or sizing — not from per-edge factor-adjusted contribution.
- **2026-05-11 | Each agent worktree's `data/trade_logs/` is independent — three worktrees can fill 25+ GB by themselves |** Today's session generated ~26 GB of trade_logs across 3 worktrees (director + agent-a + agent-b at 8.8 GB each). Caused a disk-fill incident mid-T-020 grid (ENOSPC at cell 14/25). Lesson: the three-worker pattern's hidden cost is 3× trade_log accumulation. Future hygiene: each agent's brief should include an end-of-session trade_log cleanup step, OR each worktree should rotate trade_logs older than ~7 days into an archive. Worth automating before more multi-agent sessions like today's.
- **2026-05-10 | Paused-tier parking is INERT against active-edge ensembles; lifecycle promotion is the only lift mechanism |** T-019 (full two-arm substrate-honest re-measurement on post-edge-expansion main) produced Δ Sharpe = 0.0000 in BOTH arms vs T-002 baseline. Bit-identical canon md5s across 30/30 cells. The 5 new paused edges contributed zero trades over 5 years, while pre-existing `news_sentiment_edge_v1` at the same 0.25× soft-pause weight produced 451 trades. Lesson: soft-pause (0.25× weight) gives "post-pause revival evidence" capability for edges that PREVIOUSLY were active and got lifecycle-paused. It is NOT a path to alpha contribution for newly-registered edges with sparse signal density (dividend-initiation rare events, lookback-constrained momentum, single-pair stat-arb) that lose against 6 actives producing thousands of trades. **The mechanism for converting paused inventory into headline lift is the Discovery substrate-honest gauntlet (Phase 2.10 + Gates 7/8) — edges must clear all 8 gates to reach `status='active'` where they get full weight and trade competitively.** Implication: adding more paused-tier inventory in isolation is wasted effort. Pair it with Discovery cycles that validate the inventory.
- **2026-05-10 | Foundry feature loop vectorization survives substrate-honest at 12× speedup |** T-013's 25.5× speedup on the unit-benchmark translated to ~12× wall-time speedup on the full 30-run substrate-honest measurement (50 min vs T-002's 10 hr). Lesson: engineering wins that pass canon-md5-invariance during a unit test DO compound at the full-measurement scale. Cycle cost is now materially lower, which changes the cost calculus for repeated measurements.
- **2026-05-09 | Eight-falsification pattern + engines-first reframe (NUANCED 2026-05-12) |** The cumulative falsification record (Sharpe 1.063 in-sample → MetaLearner +1.064 → wash-sale +0.670 → HRP slices 1/2/3 → Foundation Gate 1.296 → surviving-6 0.9154 → substrate-honest two-arm 0.270 → diversified-futures trend FALSIFIED) is the project converging on its actual signal as measurement geometry got more honest. **Every time the measurement gets more honest, the apparent alpha shrinks.** The 1.296 → 0.5074 → 0.9154 → 0.270 sequence is the "celebrated artifact" pattern repeating. **2026-05-12 caveat: the 0.270 leg of this sequence was itself partly an artifact of the cockpit metrics-pipeline bug. T-035 corrected it to 0.598. The "more honest measurement → smaller apparent alpha" lesson still holds for substrate/universe changes; it did NOT hold for T-002 → T-035 (substrate didn't change, only the metric reader did). Two distinct bug classes drove two distinct corrections.** Lesson: the discipline framework (deterministic harness + bootstrap CI + pre-commit kill thresholds + propose-first on Engine B) is the engineering ratchet's most valuable output. Do NOT read each falsification as "project failure"; read it as "another bug class found before risking real capital." But ALSO: most of the architecture has never operated. Engines C/D/E are scaffolding. Engine B is fixed-fraction sizing. Of course substrate-honest measurement shows weak alpha — the system has been measured as an incomplete shell. Engines-first directive anchored 2026-05-09 evening per dev review at `docs/Sessions/Other-dev-opinion/05-09-26.md`. The 0.270 substrate-honest baseline is the COMPARISON POINT against which engine-completion lift gets A/B-tested, not a death sentence.
- **2026-05-09 | Pre-commit discipline holds even when inconvenient |** T-002's pre-commit said: trigger contingent 2x2 decomposition only if Δ Sharpe ≥ +0.2. Result Δ Sharpe = +0.024, well below. Honored: NO 2x2. Despite the Sortino delta being +0.16 (4× the Sharpe delta) and showing genuine asymmetric protection in the per-year breakdown, the pre-commit was Sharpe-based; respecting it = working as designed. Lesson: pre-commits are tested by INCONVENIENCE. The Sortino observation gets logged for a separate future cycle (deployment-as-Sortino-vehicle) but doesn't get retroactively promoted as a 2x2 trigger. Counterpart of the "don't move goalposts" rule from CLAUDE.md 6th non-negotiable.
- **2026-05-09 | Sharpe headline must report `ci_low`; in-code gates use `ci_low` not point estimate |** Promoted to CLAUDE.md as the 6th non-negotiable rule (commit `4cf4909`). Operationalized via T-010 (Engine F evolution_controller.py:182, lifecycle_manager.py:66, Engine D wfo.py:184; PBO survival explicitly exempted as already distributional). Determinism canon md5 invariant pre/post the change (bootstrap is seeded). Lesson: the rule prevents implicit goalpost-moving when noise straddles a threshold — a point estimate of 0.45 with `ci_low = 0.10` does NOT clear a Sharpe < 0.4 kill thesis.
- **2026-05-09 | Cross-machine container determinism is unachievable; container-internal is the gate |** Cloud spike confirmed: Apple M-series Accelerate BLAS vs AWS Graviton OpenBLAS produce different last-decimal float results that cascade through a backtest into different canon md5s. Three Fargate workers running the same image agree bitwise with each other (cloud-internal determinism PROVEN: 3/3 reps produced canon `2b67751751a4ba515383f5791ccc4e1f`). Lesson: cloud parallelism's actual invariant is cloud-internal determinism, not cross-machine match. Project policy 2026-05-09 onward: container is canonical substrate; bare-metal becomes a debug tool only.
- **2026-05-09 | BLAS thread count drives parallel-reduction order; pin to 1 in containers |** Local container with 8 vCPU produced different canon md5 than Fargate with 1 vCPU on the same image. Diagnosis: numpy/OpenBLAS uses N threads in matrix ops, and N affects reduction order, which affects last-decimal float results. Pin OMP_NUM_THREADS / OPENBLAS_NUM_THREADS / MKL_NUM_THREADS / NUMEXPR_NUM_THREADS / VECLIB_MAXIMUM_THREADS = 1 in the Dockerfile env. Cost: container is ~6× slower than 8-thread local. Benefit: future-proofs against vCPU upgrades silently re-breaking determinism.
...
- **2026-04-23 | Regime-Conditional Walk-Forward — Mechanism Falsified OOS |** After three in-sample A/B variants (baseline 0.98, hard-kill -0.37, soft-kill 0.83, all on the same 2021-2024 window that the anchor was trained on), built a walk-forward harness at [Archive/scripts/walk_forward_regime.py](Archive/scripts/walk_forward_regime.py). Phase 1 trains the regime_tracker on 2021-2022 only (governor ON, regime_conditional ON so stats accumulate, tracker starts empty so kill-switch is inert during training). Phases 2-4 restore that OOS anchor and evaluate the three policy variants on held-out 2023-2024 with `--no-governor`. Config + governor state auto-restored on exit via try/finally.
  ...
  **Files:** `Archive/scripts/walk_forward_regime.py` (reusable walk-forward harness, `--train-start/--train-end/--eval-start/--eval-end` args), docs + memory updated three times (falsified → split-dependent → net-negative with noise).
