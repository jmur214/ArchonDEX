...
| Drawdown-gated kill switch (5/10/15% thresholds) | B | `engines/engine_b_risk/risk_engine.py:83` (config), `:940` (consumer) | no | `drawdown_kill_switch_enabled=False` default | HIGH | Charter omits this entirely; one RESOLVED line in health_check |
...
| HMM Variant C (minimal_c feature_set; 4 long-history FRED + hyg_ig_oas + copper_gold_ratio + xlp_xly_ratio) | E | `engines/engine_e_regime/regime_config.py:121` (config); model artifact `engines/engine_e_regime/models/hmm_minimal_C_v1.pkl` | no | `hmm_enabled=False` default + must select `feature_set="minimal_c"` | HIGH — Path-B HMM kill-switch foundation | T-087 validated AUC 0.887 on 12-yr; T-101 testing |
| HMM Variant A/B (legacy + minimal_a/b feature sets) | E | `engines/engine_e_regime/regime_config.py:121` (enum) | no | `hmm_enabled=False` default | MEDIUM | Multiple model artifacts shipped: `hmm_3state_v1.pkl`, `hmm_minimal_A_v1.pkl`, `hmm_minimal_B_v1.pkl`, `hmm_3state_vix_term_v1.pkl`, `hmm_monthly_v1.pkl`, `hmm_weekly_v1.pkl` |
