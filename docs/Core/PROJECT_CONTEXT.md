...
**Implication for architecture:** the goal-C target is a system that combines **statistical asymmetric capability** (small-cap momentum + 52-week breakouts + insider clusters + earnings beats + sentiment velocity, per the Moonshot Sleeve in `forward_plan_2026_04_29.md`) **with an LLM-as-analyst amplifier on top** (Phase 6 — earnings call sentiment, news theme detection, filing analysis, cross-document narrative synthesis). These are NOT parallel tracks — they're sequential layers. The statistical layer is built out FIRST until it works well; the LLM layer comes AFTER and amplifies. LLM-on-weak-bones is strictly worse than LLM-on-strong-bones. See memory `feedback_plateau_before_ai_2026_05_01.md` for the sequencing rule and `project_thematic_conviction_gap_2026_05_01.md` for what the LLM layer specifically does once it's added.

**Implication for sequencing:** there is a **multi-quarter pre-LLM expansion runway** before LLM-as-analyst becomes the right next move. That runway includes universe expansion, data-stream depth (alt-data, real news, wider Form-4 / contract awards / politician trades / patents), genuinely diverse edge categories, Discovery actually producing edges autonomously, Reform Gate 1, statistical Moonshot Sleeve build-out, cost-model completeness, and observability tooling. Most of these are partially scoped on the roadmap; the system has not yet plateaued on any of them. "Bones before paper" + "plateau before AI" together describe the same principle at two levels — earn each layer before adding the next.
