...
4-5. **`test_falsifiable_spec_*`:** now assert the **Gate-0 refusal first** (`gate_0_passed is False`, no `contribution_sharpe` — no backtest may fire on a sub-MBL window, CLAUDE.md `[NN-MBL]`), then exercise the original gauntlet-correctness spec via the escape hatch. Product right; tests predated Gate-0.
