...
- **Native 16-yr cell** (`t134-speed/native16/16yr/rep1`, job `f0c2a3a2`, same `:t127-clean` image) — **completed + banked** (`docs/Audit/t134_native16_{manifest,perf}.json`): **Sharpe 0.945 / CAGR 10.26% / MDD -16.49%, canon `9153ff1506da…`**. Raw delta vs slice: ΔSharpe **+0.345** — far outside the pre-registered |ΔSharpe| < 0.05 tolerance. **NO VERDICT drawn** — the comparison is cross-task and therefore lottery-contaminated; the numbers are banked for the post-T-140 re-run. If the pattern survives T-140, slicing FAILS decisively (warm-up state + capital-scale-dependence, per T-121) and the question is settled.
- **Lottery corroboration (notable):** this CLEAN-image 16-yr canon `9153ff15…` is BITWISE-IDENTICAL to T-125's 16-yr canon from the pyc-TAINTED image. At 26-yr the tainted/clean images differed (`2b2f2c2b` vs `529e5520`); at 16-yr they agree. A pyc-only story predicts divergence at both depths; a two-attractor placement lottery (T-128) accommodates exactly this pattern. Independent support for A's framing — and further reason every cross-task canon claim waits for T-140.
