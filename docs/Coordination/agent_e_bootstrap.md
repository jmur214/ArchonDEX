...
This doc is the briefing for Agent E's **first** message. The director will paste a short version of this into the agent's chat to start the session.

## Identity

You are **Agent E** in the multi-session coordination model:

- **Director** runs in `/Users/jacksonmurphy/Dev/trading_machine-2` (the main worktree). They coordinate work and integrate results.
- **You (Agent E)** run in `../trading_machine-agent-e/` (your worktree). You execute tasks end-to-end.
- **Agents A, B, C, D** run in their own worktrees (`../trading_machine-agent-{a,b,c}/`). Peer workers. You don't talk to them directly; the director mediates.
...
1. **FIRST COMMAND — cd into your worktree:**
   ```bash
   cd /Users/jacksonmurphy/Dev/trading_machine-agent-e
   ```
...
2. **Confirm you landed in the right worktree:**
   ...
   ```
   `pwd` must be `/Users/jacksonmurphy/Dev/trading_machine-agent-e`. If it shows `trading_machine-2` (the main worktree), STOP — the cd didn't take. Bail and tell the user.
...
3. **Read these (in order, only the first time). Use absolute paths anchored at your worktree if your Read tool defaults to a different root:**
   ...
   - This doc (`docs/Coordination/agent_e_bootstrap.md`)

4. **Check your inbox:**
   ```bash
   cat data/coordination/agent_e_inbox.md
   ```
...
1. **Read your inbox** (`data/coordination/agent_e_inbox.md`).
...
2. **Confirm workspace + rebase.** First action of any new task. The `cd` is here too in case the bash session reset between turns:
   ```bash
   cd /Users/jacksonmurphy/Dev/trading_machine-agent-e && pwd && git rev-parse --show-toplevel && git branch --show-current && git fetch origin main && git rebase origin/main
   ```
...
4. **When done:** WRITE your result to `data/coordination/agent_e_outbox.md` (overwriting the prior contents — this is a single-task slot, not an append-log) BEFORE you summarize in chat. Use the format in `PROTOCOL.md`.
...
- Bootstrap path: `docs/Coordination/agent_e_bootstrap.md` (this file)
- Protocol: `docs/Coordination/PROTOCOL.md`
- Cloud usage: `docs/Cloud/CLOUD_USAGE.md`
- Your inbox: `data/coordination/agent_e_inbox.md`
- Your outbox: `data/coordination/agent_e_outbox.md`
