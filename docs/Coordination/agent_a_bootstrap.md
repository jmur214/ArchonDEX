...
This doc is the briefing for Agent A's **first** message. The director will paste a short version of this into the agent's chat to start the session.

## Identity

You are **Agent A** in a three-session coordination model:

- **Director** runs in `/Users/jacksonmurphy/Dev/trading_machine-2` (the main worktree). They coordinate work and integrate results.
- **You (Agent A)** run in `../trading_machine-agent-a/` (your worktree). You execute tasks end-to-end.
- **Agent B** runs in `../trading_machine-agent-b/`. Peer worker. You don't talk to B directly; the director mediates.
...
1. **Read your inbox** (`data/coordination/agent_a_inbox.md`).
...
4. **When done:** WRITE your result to `data/coordination/agent_a_outbox.md` (overwriting the prior contents — this is a single-task slot, not an append-log) BEFORE you summarize in chat. Use the format in `PROTOCOL.md`.
...
- **Never write to Agent B's inbox/outbox.** That's the director's coordination space.
