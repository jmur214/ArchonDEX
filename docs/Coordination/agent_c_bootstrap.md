...
This doc is the briefing for Agent C's **first** message. The director will paste a short version of this into the agent's chat to start the session.

## Identity

You are **Agent C** in a three-session coordination model:

- **Director** runs in `/Users/jacksonmurphy/Dev/trading_machine-2` (the main worktree). They coordinate work and integrate results.
- **You (Agent C)** run in `../trading_machine-agent-c/` (your worktree). You execute tasks end-to-end.
- **Agent A** runs in `../trading_machine-agent-a/` and **Agent B** in `../trading_machine-agent-b/`. Peer workers. You don't talk to them directly; the director mediates.
...
1. **FIRST COMMAND — cd into your worktree:**
   ```bash
   cd /Users/jacksonmurphy/Dev/trading_machine-agent-c
   ```
...
2. **Confirm you landed in the right worktree:**
   ...
   ```
   `pwd` must be `/Users/jacksonmurphy/Dev/trading_machine-agent-c`. If it shows `trading_machine-2` (the main worktree), STOP — the cd didn't take. Bail and tell the user.
...
3. **Read these (in order, only the first time). Use absolute paths anchored at your worktree if your Read tool defaults to a different root:**
   ...
   - This doc (`docs/Coordination/agent_c_bootstrap.md`)

4. **Check your inbox:**
   ```bash
   cat data/coordination/agent_c_inbox.md
   ```
...
1. **Read your inbox** (`data/coordination/agent_c_inbox.md`).
...
2. **Confirm workspace + rebase.** First action of any new task. The `cd` is here too in case the bash session reset between turns:
   ```bash
   cd /Users/jacksonmurphy/Dev/trading_machine-agent-c && pwd && git rev-parse --show-toplevel && git branch --show-current && git fetch origin main && git rebase origin/main
   ```
...
4. **When done:** WRITE your result to `data/coordination/agent_c_outbox.md` (overwriting the prior contents — this is a single-task slot, not an append-log) BEFORE you summarize in chat. Use the format in `PROTOCOL.md`.
...
- **Never write to another agent's inbox/outbox.** That's the director's coordination space.
...
- **Do NOT write `docs/State/TASK_LEDGER.md`** (protocol T-114, 2026-06-06). Every agent branch appending a ledger row guarantees a merge conflict. Instead, put your proposed ledger row in your OUTBOX under "Proposed TASK_LEDGER row"; the director writes it on main at merge time. Same for any single append-only shared file (e.g. CURRENT_STATE rolling sections) — propose in outbox, director writes.
...
- Bootstrap path: `docs/Coordination/agent_c_bootstrap.md` (this file)
- Protocol: `docs/Coordination/PROTOCOL.md`
- Cloud usage: `docs/Cloud/CLOUD_USAGE.md`
- Your inbox: `data/coordination/agent_c_inbox.md`
- Your outbox: `data/coordination/agent_c_outbox.md`
