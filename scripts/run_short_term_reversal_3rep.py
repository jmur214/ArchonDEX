"""
scripts/run_short_term_reversal_3rep.py
=========================================
T-2026-05-11-030: short_term_reversal_v1 3-rep re-measurement.

Today's threshold-calibration audit flagged STR as the closest-miss to
the t > 2 bar (t = +1.76, α = +3.39% annualized). T-020 noted a
suspicious 2022 zero-Sharpe cell. T-029's per-regime decomp classified
STR as UNIFORMLY NOISY but ran on the existing T-020 trade logs without
re-measurement.

This dispatch: 3 reps × 5 years × 1 edge (short_term_reversal_v1) in
isolation (`exact_edge_ids=[short_term_reversal_v1]`) on substrate-
honest, status='active' inside isolation (full weight, no soft-pause
0.25× multiplier).

Key question: was 2022's zero-Sharpe deterministic noise (3 reps will
produce 3 different canon md5s if non-deterministic) or genuine
regime-conditional behavior (will be consistent across reps but
anomalously zero compared to other years)?

Reuses `scripts.run_per_edge_isolation` machinery — pre-import + populate-
inside-isolation + flip-to-active-in-context.

Output:
  docs/Audit/short_term_reversal_rerun_2026_05_11.md (+ .json)
"""
...
RESULTS_DIR = ROOT / "data" / "measurements" / "short_term_reversal_3rep_2026_05_11"
...
def _reexec_if_hashseed_unset() -> None:
    if os.environ.get("PYTHONHASHSEED") != "0":
        ...
        os.execv(
            sys.executable,
            [sys.executable, "-m", "scripts.run_short_term_reversal_3rep",
             *sys.argv[1:]],
        )
