"""scripts/run_bab_gauntlet_t123.py
====================================
T-2026-06-06-123 — run the Discovery gauntlet on the new VRP literature edge.
...
Output: data/measurements/bab_gauntlet_t123/result.json
Determinism: PYTHONHASHSEED=0 via re-exec.

Usage: PYTHONHASHSEED=0 python -m scripts.run_bab_gauntlet_t123 [--start 2021-01-01 --end 2025-12-31]
"""
...
OUTPUT_DIR = ROOT / "data" / "measurements" / "bab_gauntlet_t123"
...
def _reexec_if_hashseed_unset() -> None:
    if os.environ.get("PYTHONHASHSEED") != "0":
        ...
        os.execv(sys.executable,
                 [sys.executable, "-m", "scripts.run_bab_gauntlet_t123", *sys.argv[1:]])
...
def _build_data_map(start: str, end: str):
    ...
    print(f"[T123] universe: mode={uni_info['mode']} "
    ...
    print(f"[T123] data_map: {len(data_map)} tickers", flush=True)
    return data_map, uni_info


def _candidate_spec() -> dict:
    from engines.engine_a_alpha.edges.betting_against_beta_edge import (
        BettingAgainstBetaEdge,
    )
    return {
        "edge_id": "betting_against_beta_v1",
        "module": BettingAgainstBetaEdge.__module__,
        "class": BettingAgainstBetaEdge.__name__,
        "category": BettingAgainstBetaEdge.CATEGORY,
        "params": dict(BettingAgainstBetaEdge.DEFAULT_PARAMS),
        "status": "candidate",
        "version": "1.0.0",
        "origin": "manual_dispatch_t123",
    }
...
def main() -> int:
    ...
    t0 = time.time()
    print(f"[T123] window: {args.start} → {args.end}", flush=True)
    ...
    print(f"[T123] candidate: {cand['edge_id']} (status={cand['status']})", flush=True)
    ...
    print(f"\n[T123] gauntlet done in {elapsed/60:.1f} min → {OUTPUT_PATH}", flush=True)
