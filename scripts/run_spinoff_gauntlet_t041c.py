"""scripts/run_spinoff_gauntlet_t041c.py
=========================================
T-041c-archive Part B: re-run the 8-gate Discovery gauntlet on
`spinoff_reversion_v1` with a clean baseline (paused entry archived).

Mirrors T-041b's driver exactly, only changes the output dir so the
T-041b result is preserved for direct comparison. The detector,
universe wiring, candidate spec, and validate_candidate invocation
are identical — only the registry state (spinoff_reversion_v1 now
status=archived) differs, which removes the 0.25× baseline weight
that T-041b's audit identified as a confound.
...
OUTPUT_DIR = ROOT / "data" / "measurements" / "spinoff_reversion_t041c_archive_gauntlet"
...
def _reexec_if_hashseed_unset() -> None:
    if os.environ.get("PYTHONHASHSEED") != "0":
        ...
        os.execv(
            sys.executable,
            [sys.executable, "-m", "scripts.run_spinoff_gauntlet_t041c",
             *sys.argv[1:]],
        )
