"""
scripts/run_vol_target_arms_regime_t055e.py
============================================
T-2026-05-23-055e — regime-conditional vol-target A/B verification.

Reuses the T-055d harness structure:
  * Arm 0 (control): vol-target OFF (REUSED from T-055d arm0 results,
    which itself was reused from T-055c — identical config under OFF flag).
  * Arm 1 (treatment): vol-target ON, EWMA λ=0.94 (winner from T-055d)
    PLUS regime_aware=True. Engine E's advisory["regime_summary"] gates
    the target multiplier per the dispatch spec (benign 1.0, cautious
    0.85, stressed 0.60, crisis 0.40).
...
Usage:
  PYTHONHASHSEED=0 python -m scripts.run_vol_target_arms_regime_t055e --full

Hypothesis under test (per T-055e dispatch):
  * 2022 EWMA degradation (-0.594) should soften toward OFF (0.294)
    because the stress-window degross is muted via 0.60× multiplier.
  * 2024 fragility rescue (+1.622) should be preserved or amplified.
  * 2025 vol-shock trap fix (-0.128 EWMA vs -0.942 rolling) should hold.

  Net target: ci_low(Δ Sharpe) crosses 0 → T-055b flag-flip
  becomes user-decision-gate defensible.
"""
...
RESULTS_DIR = ROOT / "data" / "measurements" / "vol_target_regime_t055e_2026_05_23"
T055D_RESULTS_DIR = ROOT / "data" / "measurements" / "vol_target_ewma_t055d_2026_05_22"
...
# Arm 1 — EWMA (T-055d winner) + regime_aware ON.
VOL_TARGET_ARM1_REGIME_PATCH = {
    ...
    "portfolio_vol_target_cautious_multiplier": 0.85,
    "portfolio_vol_target_stressed_multiplier": 0.60,
    "portfolio_vol_target_crisis_multiplier": 0.40,
}
...
def _reexec_if_hashseed_unset() -> None:
    if os.environ.get("PYTHONHASHSEED") != "0":
        ...
        os.execv(
            sys.executable,
            [sys.executable, "-u", "-m", "scripts.run_vol_target_arms_regime_t055e", *sys.argv[1:]],
        )


@contextmanager
def vol_target_regime_patch(enabled: bool) -> Iterator[None]:
    original = RISK_CONFIG_PATH.read_text()
    try:
        cfg = json.loads(original)
        if enabled:
            for k, v in VOL_TARGET_ARM1_REGIME_PATCH.items():
                cfg[k] = v
        else:
...
def _execute_grid(arm_label: str, years: list[int], reps: int,
    ...
    with vol_target_regime_patch(vol_target_on):
        for year in years:
            for rep in range(1, reps + 1):
                ...
                try:
                    ...
                    record = {
                        ...
                        "regime_aware": vol_target_on,  # arm1 only
                        "year": year,
                        "rep": rep,
                        "run_id": run_id,
...
def main() -> int:
    ...
    parser.add_argument("--full", action="store_true",
                        help="Run arm1 regime-aware (15 backtests); arm0 reused from T-055d.")
    ...
    if not ISOLATED_ANCHOR.exists():
        print("[T-055E] No anchor — run `python -m scripts.run_isolated --save-anchor` first.",
              file=sys.stderr)
        return 1
    ...
    if args.full:
        # Reuse T-055d arm0 — identical config (vol-target OFF).
        RESULTS_DIR.mkdir(parents=True, exist_ok=True)
        t055d_arm0 = T055D_RESULTS_DIR / "arm0_results.json"
        target_arm0 = RESULTS_DIR / "arm0_results.json"
        if t055d_arm0.exists() and not target_arm0.exists():
            import shutil as _sh
            _sh.copy2(t055d_arm0, target_arm0)
            print(f"[T-055E] Reused T-055d arm0 results from {t055d_arm0}", flush=True)
        print(f"\n[T-055E] ARM 1 (vol-target ON, EWMA + regime_aware) — years={years} reps={args.reps}",
              flush=True)
        ...
        sentinel.write_text(f"T-055e grid complete at {datetime.now().isoformat(timespec='seconds')}\n")
        print(f"[T-055E] FULL DONE — see {sentinel}", flush=True)
        return 0
